...
resource "cloudflare_access_application" "prod_vps_to_op_network" {
  ...
  domain                    = "*.tcp-prod-network.${local.root_domain}"
  type                      = "self_hosted"
...
resource "cloudflare_access_service_token" "prod_linode_to_onp" {
  zone_id    = local.cloudflare_zone_id
  name       = "Linode (for Production Network)"
...
resource "cloudflare_access_policy" "prod_linode_to_onp" {
  application_id = cloudflare_access_application.prod_vps_to_op_network.id
  zone_id        = local.cloudflare_zone_id
  ...
  include {
    service_token = [
      cloudflare_access_service_token.prod_linode_to_onp.id
    ]
  }
}
